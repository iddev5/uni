...
for idx, medoid in enumerate(medoids):
    G.nodes[medoid]['is_medoid'] = True
    G.nodes[medoid]['cluster'] = f"Cluster {idx+1}"

for idx, (medoid, points) in enumerate(clusters.items()):
    for p in points:
        G.nodes[p]['cluster'] = f"Cluster {idx+1}"
        if p != medoid:
            G.add_edge(medoid, p)

node_colors = ['red' if G.nodes[n].get('is_medoid') else 'lightblue' for n in G.nodes()]
node_sizes = [600 if G.nodes[n].get('is_medoid') else 300 for n in G.nodes()]
node_labels = {n: f"{n}\n{G.nodes[n]['cluster']}" for n in G.nodes()}

plt.figure(figsize=(8, 7))
pos = {node: node for node in G.nodes()}
nx.draw(G, pos, labels=node_labels, node_color=node_colors, node_size=node_sizes, font_size=8, font_weight='bold')
plt.title("K-Medoids Clusters with Medoids Highlighted")
plt.show()
